...
<%
	session.setAttribute("returnUrl", "/bbs/inquiry.do");
...
<div class="sub-container">
				...
				<div class="section-inner">
					...
					<div class="tbl-wrap">
									...
									<c:forEach var="inquiry_list" items="${inquiry_list}" varStatus="status">
										<tr>
											...
											<td class="col wid* post-tit tl">
												<a href="/bbs/inquiry_view.do?nttId=${inquiry_list.nttId}" class="dot">${inquiry_list.nttSj}</a>
											</td>
											<td class="col colnone">
												<c:if test="${inquiry_list.atchFileId ne null && inquiry_list.atchFileId ne ''  && inquiry_list.atchFileId ne undefined}">
													<i class="ico-addfile">첨부파일</i>
												</c:if>
											</td>
											<td class="col colnone">${inquiry_list.ntcrNm}</td>
											<td class="col day colnone">${fn:substring(inquiry_list.frstRegistPnttm,0,11)}</td>
											<td class="col colnone"><i class="ico-view"></i>${inquiry_list.rdcnt}</td>
										</tr>
									</c:forEach>
								</c:if>
						...
						<ul class="paginations">
							<c:if test="${pictVO.pageNumber ne 1}">
								<li><a href="/bbs/inquiry.do?search_text=${param.search_text}&pageNumber=1"><img src="/_user/img/First.png" alt=""></a></li>
								<li><a href="/bbs/inquiry.do?search_text=${param.search_text}&pageNumber=${pictVO.pageNumber - 10 < 1 ? 1 : pictVO.pageNumber - 10}"><img src="/_user/img/Prev.png" alt=""></a></li>
							</c:if>	
						
							<c:forEach var="i" begin="${pictVO.startPage}" end="${pictVO.endPage}">
								<c:if test="${i eq pictVO.pageNumber}">
									<li class="active"><a href="/bbs/inquiry.do?search_text=${param.search_text}&pageNumber=${i}" >${i}</a></li>
								</c:if>
								<c:if test="${i ne pictVO.pageNumber}">
									<li><a href="/bbs/inquiry.do?search_text=${param.search_text}&pageNumber=${i}" >${i}</a></li>
								</c:if>
							</c:forEach>	
					
							<c:if test="${pictVO.lastPage ne pictVO.pageNumber}">
								<li><a href="/bbs/inquiry.do?search_text=${param.search_text}&pageNumber=${pictVO.pageNumber + 10 > pictVO.lastPage ?  pictVO.lastPage : pictVO.pageNumber + 10}"><img src="/_user/img/Next.png" alt=""></a></li>
								<li><a href="/bbs/inquiry.do?search_text=${param.search_text}&pageNumber=${pictVO.lastPage}"><img src="/_user/img/Last.png" alt=""></a></li>
							</c:if>
						</ul>
	...
	<script>
		function search(){
			$("#listFrm").attr("action", "/bbs/inquiry.do");
			$("#listFrm").submit();
		}
	</script>
</div>
